...
void GameScene::initEnemy(sf::Vector2u windowSize)
{
	for (int i = 0; i < totalEnemySize; i++)
	{
		...
		if (type == EnemyType::Sword)//intialise a sword enemy
		{
			...
			melee->initialise(sf::Vector2f((float)-1000, (float)(windowSize.y / 2) + 70), m_playerbody, m_shadow, m_meleeWeapon, i);//initialising enemy points
			enemyMelee.push_back(melee);
			waveManager.addToWave(melee, rand() % 2 + 1);
		...
		else if (type == EnemyType::Gun)//intialise a Gun enemy
		{
			...
			waveManager.addToWave(gun, rand() % 2 + 1);
			entities.push_back(enemyGun[i]);//placing enemy pointers within array
		}
...
void GameScene::gameUpdate(sf::Time elapsedTime, sf::Vector2u windowSize)
{
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::R) && debugPressed == false)//fix for when the player kills all enemies on screen
	{
		waveManager.setSpawnPos(waveManager.getSpawner().left + 100, waveManager.getSpawner().top);
		debugPressed = true;
	}
	...
	player.update(elapsedTime, sf::Vector2f(windowSize), &projectileManager);

	enemyUpdate(elapsedTime, windowSize);
...
void GameScene::enemyUpdate(sf::Time elapsedTime, sf::Vector2u windowSize)
{
	sf::IntRect viewportRect = sf::IntRect((int)(camera.getCenter().x - camera.getCenter().x),
	...
	for (size_t i = 0; i < enemyMelee.size(); i++)
	{
		...
		else if (!enemyMelee[i]->isAlive() && !enemyMelee[i]->isActivated())
		{
			//std::swap(enemyMelee[i], enemyMelee.back());
			enemyMelee.erase(std::remove(enemyMelee.begin(), enemyMelee.end(), enemyMelee[i]), enemyMelee.end());
			//delete from vector array if not alive
		}
	}
	...
	for (size_t i = 0; i < enemyGun.size(); i++)
	{
		...
		{
			enemyGun.erase(std::remove(enemyGun.begin(), enemyGun.end(), enemyGun[i]), enemyGun.end());
			//delete from vector array if not alive
		}
...
void GameScene::gameDraw(sf::RenderWindow * window)
{
	...
	spawne.setPosition((float)waveManager.getSpawner().left, (float)waveManager.getSpawner().top);
	spawne.setSize(sf::Vector2f((float)waveManager.getSpawner().width, (float)waveManager.getSpawner().height));
